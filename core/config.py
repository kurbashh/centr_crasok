...
class Settings(BaseSettings):
    ...
    # Максимум символов в одном сообщении пользователя.
    # Сообщения длиннее лимита отклоняются ДО вызова AI — экономим токены
    # и избегаем обрезанных ответов от Gemini.
    #
    # Рекомендуемые значения:
    #   800  — строгий режим (только конкретные вопросы)
    #   1200 — стандарт (вопрос + контекст ремонта)       ← дефолт
    #   2000 — мягкий (клиент вставляет список помещений)
    #
    # Добавь в .env: MAX_INPUT_LENGTH=1200
    max_input_length: int = 1200   # MAX_INPUT_LENGTH
